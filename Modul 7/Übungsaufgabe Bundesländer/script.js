...
function render(){
    ...
    for (let i = 0; i < bundeslaender.length; i++) {
        ...
        content.innerHTML +=/*html*/ `
            <div class="card">
                <h2>${land['name']}</h2>
                <div>${land['population']} Millionen</div>
                <div><a href="${land['url']}" target="_blank">${land['url']}</a></div>
                <div id="landcontent${i}">${land['comments']}</div>
...
function addComment(index){
    let commentInput = document.getElementById(`input${index}`);
    let comment = commentInput.value;

   
    bundeslaender[index]['comments'].push(comment);
